...
const buttons = () => {
  const domString = `
  <button id="cat-btn" style="background-color: lightblue;">Cats</button>
  <button id="dog-btn" style="background-color: lightgreen;">Dogs</button>
  <button id="dino-btn" style="background-color: orange;">Dinos</button>
  <button id="allpets-btn" style="background-color: pink;">All Pets</button>
  `;

  renderToDom("#allBtns", domString);
};
...
//function that filters the pets by type
const filterPets = (array, type) => {
  return array.filter((petObj) => petObj.type === type);
};

//function handling clicks
const handleButtonClick = (event) => {
  if (event.target.id === "allpets-btn") {
    const allPets = filterPets(pets);
    petBuilder(pets);
  } if (event.target.id === "cat-btn") {
    const cats = filterPets(pets, "cat");
    petBuilder(cats);
  } if (event.target.id === "dog-btn") {
  const dogs = filterPets(pets, "dog");
  petBuilder(dogs);
  } if (event.target.id === "dino-btn") {
    const dinos = filterPets(pets, "dino");
    petBuilder(dinos);
  }
};
